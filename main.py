...
from big_sub_category import big_category, labelling
...
async def clear():
    async with await create_connection() as conn:
        async with conn.cursor() as cursor:
            await cursor.execute("truncate table cafe_article")
            await cursor.execute("truncate table repair_order")
            await cursor.execute("truncate table ca_big_category")
            await cursor.execute("truncate table ro_big_category")
            await cursor.execute("truncate table ca_sub_category")
            await cursor.execute("truncate table ro_sub_category")
            await cursor.execute("truncate table ca_keyword")
            await cursor.execute("truncate table ro_keyword")
            await cursor.execute("truncate table ca_keyword")
            await cursor.execute("truncate table ro_keyword")
            await cursor.execute("truncate table similarity")

            values = [(big_cate, 0) for big_cate in big_category.values()]
            await cursor.executemany("insert into ca_big_category (cate_name, count)  values (%s, %s)", values)
